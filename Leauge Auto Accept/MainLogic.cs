...
namespace Leauge_Auto_Accept
{
    internal class MainLogic
    {
        ...
        public static bool isMsgOn = false;
        ...
        private static async Task SendTelegramAsync(string message, string statusKey)
        {
            if (lastStatus == statusKey) 
                return; // already sent this type of message
        ...
        private static async void handleMatchmakingAccept()
        {
            if (!Settings.cancelQueueAfterDodge)
            {
                ...
                if (isMsgOn)
                {
                    await SendTelegramAsync("Accepted ready check ✅", "accepted");
                }
            ...
            if (lastChatRoom != "")
            {
                ...
                if (isMsgOn)
                {
                    await SendTelegramAsync("Declined ready check ❌ (due to cancelQueueAfterDodge)", "declined");
                }
            }
            else
            {
        ...
        private static void handleQueueRestart(string phase)
        {
            switch (phase)
            {
                case "Matchmaking" when phase != lastPhase:
